...
import {TableAction, TableActionEvent, TableDataType, TableMapping} from './table.component.constants';
...
export class TableComponent implements OnInit {
    ...
    onActionClick(event: string, object: any, row: any = null) {
        if (row) {
            this.actionClick.emit({event: event, object: object, row: row});
            return;
        }
        this.actionClick.emit({event: event, object});
    }

    getValue(col: any, row: any, editFormat = false, id: number = null) {
        let value;
        try {
            value = col.valueKey.split('.').reduce((o, d) => o[d], row);
        } catch (e) {
            return null;
        }
    ...
    onTableAction($event: TableActionEvent, row: any) {
        this.onActionClick($event.event, $event.object, row);
    }

    cloneObject(): any {
        const clone: any = {};
        // tslint:disable-next-line:forin
        for (const key of this.mappingKeys.filter(m => m.type !== TableDataType.SUBTABLE)) {
            this.arrayNotationToObject(key.valueKey, null, clone);
        }
        if (this.createAdditionalParams) {
            return {...clone, ...this.createAdditionalParams};
        }
        return clone;
    }
